...
public class LogFileActionEngineFactory implements LogFileEngine {
    ...
    public LogFileActionEngineFactory(@NonNull Context context) {
        if (context == null) {
    ...
    public void writeToFile(File logFile, @LogLevel.LogLevelType int logLevel, String logContent, LogFileParam params) {
        if (buffer == null) {
            synchronized (LogFileActionEngineFactory.class) {
                if (buffer == null) {
                    File bufferFile = new File(context.getFilesDir() , ".log4aCache-a");
