...
import static com.tan.log.LogLevel.TYPE_INFO;
import static com.tan.log.LogLevel.TYPE_VERBOSE;
import static com.tan.log.LogLevel.TYPE_WARM;
import static com.tan.log.LogLevel.TYPE_WTF;
import static com.tan.log.utils.Utils.DIVIDER_BOTTOM;
import static com.tan.log.utils.Utils.DIVIDER_CENTER;
import static com.tan.log.utils.Utils.DIVIDER_NORMAL;
import static com.tan.log.utils.Utils.DIVIDER_TOP;
import static com.tan.log.utils.Utils.printDividingLine;

import android.text.TextUtils;
import android.util.Log;

import com.tan.log.file.LogFileParam;
import com.tan.log.parser.LocalParserManager;
    ...
    private void writeToFile(String tagName, String logContent, @LogLevel.LogLevelType int logLevel) {
        ...
        if (logLevel <= TYPE_ACTION) {
            ...
            if (log2FileConfig.getEngine() != null) {
                ...
                log2FileConfig.getEngine().writeToFile(logFile, logLevel, logContent, param);
            } else {
                throw new IllegalArgumentException("LogFileEngine must not Null");
            }
        } else if (logLevel == TYPE_HTTP) {

            if (log2FileConfig.getHttpEngine() != null) {
                File logFile = new File(log2FileConfig.getHttpLogPath(), log2FileConfig.getLogHttpFormatName());
                LogFileParam param = new LogFileParam(System.currentTimeMillis(), logLevel,
                        Thread.currentThread().getName(), tagName);
